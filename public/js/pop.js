...
window.pop = {
    ...
    */
    confirm: function(title, desc, fn,cancelFn) {
    ...
    */
    tips: function () {
        ...
        _doc.on('touchstart','.icon-question-circle',function(ev){//为此图标绑定事件
            var _this = $(this);
            ...
            var popContent = $('<p><span>' + text + '</span><i></i></p>');
            popTips.append(popContent).appendTo(_body);
            var arrThisOffset = getElementOffset(_this);//获得此图标距离页面顶端的距离和左边的距离
            var arrTipOffset = [popTips.width(),popTips.height()]; //获得tip的宽高
            var tipFinalOffset = [
                arrThisOffset[0] - (arrTipOffset[0]/2) + (_this.width()/2),//图标左偏移距离-tip宽度的一般+图标宽度的一半
                arrThisOffset[1] - arrTipOffset[1] - 10//图标上偏移距离-tip高度
            ];
            if(tipFinalOffset[0]<0){
                var arrowDown = popTips.find('i');
                arrowDown.css({'left':arrThisOffset[0]+_this.width()/2-2});
            }            
            popTips.css({
                'left': tipFinalOffset[0]<0?1:tipFinalOffset[0],
                'top': tipFinalOffset[1] - 5
            });
            if( tipFinalOffset[0]+arrTipOffset[0]>clientwidth ){
              var arrowDown = popTips.find('i');
              arrowDown.css({'left':arrThisOffset[0]+_this.width()/2-1-clientwidth+arrTipOffset[0]});
              popTips.css({
                'left' :'auto',
                'right': 1,                
                'top': tipFinalOffset[1] - 5
...
*/
function getElementOffset(domobj){
    return [domobj.offset().left,domobj.offset().top];
}
//渲染时初始化
$(function(){
  $(document.body).on('touchend', '.icon-question-circle', function(ev){
    pop.alert('提示', $(this).data('pop-tips'), 1, 0, '确定', 0);
    ev.preventDefault();
    return false;
  });
  //pop.tips();
